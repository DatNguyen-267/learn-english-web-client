...
function ListenQuestionEnd({ list_question, question_true, ls, part_id }) {
  ...
  return (
    <div className="lsquestion-end">
      <div className="lsquestion-end-main-title">Chúc mừng bạn đã hoàn thành</div>
      <div className="lsquestion-end-sub-title">
        <span className="sub-title-total-number">Tổng số câu: {total_question}</span>
        <br></br>
        <span className="sub-title-correct-number">Số câu đúng: {question_true}</span>
      </div>
      <div className="lsquestion-end-option">
        <button onClick={handleBack} className="lsquestion-end-btn-back">
          Thử lại
        </button>
        <button onClick={handleNext} className="lsquestion-end-btn-next">
          Tiếp tục
        </button>
      </div>
    </div>
