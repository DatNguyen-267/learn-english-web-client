...
export const VocaLearning_2 = ({ topic, courseId }) => {
    ...
    if (topic) {
      ...
      topic.list_word.forEach((word, index) => {
        if (index === 0) {
          ...
          newListData.push({
            type: "Question", answer: question_2.answer, index: count,
            isCorrect: undefined
          })
          count++
          newList.push({
          ...
          newListData.push({
            type: "Question", answer: question_1.answer, index: count,
            isCorrect: undefined
          })
          count++
  ...
  const handleNext = async (e) => {
    if (listData[currentIndex].type === "Word" && listCpn[currentIndex].err === undefined)
      setProgress(progress => progress + Math.round(100 * (1 / (topic.list_word.length * 2))))
    else if (listData[currentIndex].type === "Question" && listData[currentIndex].isCorrect) {
      setProgress(progress => progress + Math.round(100 * (1 / (topic.list_word.length * 2))))
    }
  ...
  const handleAnswerQ = async (e) => {
    ...
    const indexListData = listData.findIndex((item) =>
      (item.index == index))
          ...
          for (let i = currentIndex + 2; i < listCpn.length; i++) {
            if ((listCpn[i].type === "word" && listCpn[i + 1].type === "word")
              ...
              listData = insert(listData, i + 2, {
                type: "Question", answer: question.answer, index: listData.length,
                isCorrect: undefined
              })
              isInsert = true
              break;
  ...
  const handleIDontKnow = async (e) => {
    if (listData[currentIndex].isCorrect === undefined) {
      ...
      const allBtn = document.querySelectorAll(`[data-index="${listData[currentIndex].index}"]`);
      for (let i = 0; i < allBtn.length; i++) {
        if (answer.toUpperCase() ===
          allBtn[i].querySelector('.quick-q__answer-title').textContent.toUpperCase()) {
    ...
    <div>
      ...
      {!isEnd && <div className="learn__body">
            ...
            (listCpn.length > 0 && listCpn[currentIndex].type === "word") && (
              <div className={listCpn[currentIndex].className}>
                <VocaInfo
                  ...
                  err={listCpn[currentIndex].err} ></VocaInfo>
              </div>)}
          {
        ...
        <div className='learn__body-action'>
          ...
          {listData.length > 0 && listData[currentIndex].type === "Question" &&
            listData[currentIndex].isCorrect === undefined &&
            <button className="btn learn__body-action-btn-dont-no" onClick={handleIDontKnow}>
              <span>Tôi không biết</span>
            </button>}
          {isCorrect &&
            <button className="btn learn__body-action-btn-correct" onClick={handleIDontKnow}>
              <i className="fa fa-check"></i>
              <span>Chính xác!</span>
            </button>}
          {isCorrect === false &&
            <button className="btn learn__body-action-btn-incorrect" onClick={handleIDontKnow}>
