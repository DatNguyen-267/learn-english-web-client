...
export const VocaLearning = () => {
  ...
  const handleNext = (e) => {
    ...
    if (
      currentData.countWord + 1 === stack.words.length &&
    ...
    if (!firstItem.style.marginLeft) firstItem.style.marginLeft = `-100%`;
    else firstItem.style.marginLeft = `${parseInt(firstItem.style.marginLeft) - 100}%`;
  };
  const renderQuestion = () => {
    if (rootTopic.data && rootTopic.isUpdate == true) {
      rootTopic.data.list_word.forEach((element, index) => {
        ...
        setStack({
          ...stack,
          questions: stack.questions.push({
            q: element.english,
  ...
  useEffect(() => {
    ...
    if (stack.words.length > 0 && stack.questions.length > 0) {
      ...
      listComponent.push({
        component: 'QuickQuestionABCD',
        data: stack.questions[0],
        className: 'slide-item',
      });
    ...
    if (isCorrect) {
      if (list[index - 1].type === 'Word') {
        if (list[index - 2].type === 'Word') {
          listComponent.push({
            component: 'QuickQuestionABCD',
            data: stack.questions[currentData.countQ + 1],
            className: 'slide-item',
          });
          setListComponent([...listComponent]);
        } else if (currentData.countQ % 2 === 0) {
          ...
          if (stack.words.length - currentData.countWord - 1 === 0) {
            ...
            listComponent.push({
              component: 'VocaInfo',
              data: stack.words[currentData.countWord + 1],
              className: 'slide-item',
            });
            listComponent.push({
              component: 'VocaInfo',
              data: stack.words[currentData.countWord + 2],
              className: 'slide-item',
            });
            listComponent.push({
              component: 'QuickQuestionABCD',
      ...
      listComponent.push({
        component: 'VocaInfo',
        data: stack.words[currentData.countQ],
        className: 'slide-item',
      });
      listComponent.push({
        component: 'QuickQuestionABCD',
        data: stack.questions[currentData.countQ],
        className: 'slide-item',
      });
      setListComponent([...listComponent]);
    }
  ...
  const handleAnswerQ = (e) => {
    ...
    if (answer === btn.querySelector('.quick-q__answer-title').textContent) {
      ...
      for (let index = 0; index < allBtn.length; index++) {
        if (
          answer === allBtn[index].querySelector('.quick-q__answer-title').textContent
        ) {
      ...
      {!isEnd && (
        <div className="learn__body">
              ...
              {listComponent.length > 0 &&
                listComponent.map((item, index) => (
                  <div className={item.className} key={index}>
                    {item.component === 'VocaInfo' ? (
                      <VocaInfo data={item.data}></VocaInfo>
          ...
          <div className="learn__body-action">
            {list[index] &&
              (list[index].type === 'Word' ||
                (list[index].type === 'Question' && list[index].isCorrect !== null)) && (
