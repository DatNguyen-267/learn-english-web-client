  ...
  const handleNext = () => {
    try {
      const checkCompleteTopic = async() => {
        let res = await axios.get(`${SERVER_URL}/courses/voca/checkCompleteTopic/${courseId}/${topic._id}`,{
          headers: { Authorization: token }
        })
        if (res.data === "success") {
          navigate(`/learning/voca?course-id=${courseId}&topic-id=${res.data.next_topic}&voca-course-name=`)
        }
      }
      checkCompleteTopic()
    } catch (error) {
  ...
  return (
    <div>
      <div className='voca-ln-end__header'>
        ...
        <span className='voca-ln-end__header-sub-title'>Hãy xem lại những từ này một lần nữa</span>
        <div className='voca-ln-end__header-option'>
          <button onClick={handleBack} className='voca-ln-end__header-btn-back'>Quay về</button >
      ...
      <div className='voca-ln-end__container'>
        <div className='voca-ln-end__container-list'>
          ...
          {topic && topic.list_word.map((item, index) => (
            <div className="voca-ln-end__container-item">
              ...
              <div className='voca-ln-end__item-option'>
                <i
                  ...
                  className={`${item.isSave ? "active" : ""} voca-ln-end__item-option-heart fas fa-heart`}></i>
                <i
