...
function SpeakGrammarPage() {
  const param = useParams();
  const id = param.id;
  const [list_menu, setlistmenu] = useState()
  const dispatch = useDispatch();
  const data = useSelector((state) => state.speakGrammar.data);
  
  useEffect(() => {
    dispatch(actions.findSpeakGammmarRequest(id));
  }, [dispatch]);
  useEffect(() =>{
  ...
  return (
    <div className="grid wide">
      <div className="speak-grammar-page">
        <div className="speak-grammar-page__header">
          <div className="speak-grammar-page__header-content">
            <div className="speak-grammar-page__header-content-title">
              <h1>{grammartopic.name}</h1>
            </div>
          </div>
        </div>
        <div className="speak-grammar-page-content">
...
export default SpeakGrammarPage;
