    ...
    try {
      const getCourse = async () => {
        const res = await axios.get(
          `${SERVER_URL}/courses/get-course/${courseId}`
        );
        setCourse(res.data[0]);
      };
      const getListWord = async () => {
        const res = await axios.get(
          `${SERVER_URL}/courses/get-topic/${topicId}`
        );
    ...
    <div className="ls-wd-pg">
      <div className="grid wide">
        <div className="ls-wd-pg__head">
          <h2>600 từ vựng toeic - Contract</h2>
          <button className="ls-wd-pg-btn-back" onClick={handelBack}>
            Quay lại
          </button>
        </div>
        <div className="ls-wd-pg__ls row">
          {listWord !== undefined &&
            listWord.list_word.map((item, index) => (
